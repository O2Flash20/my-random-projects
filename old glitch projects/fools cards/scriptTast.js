...
function decodeURLParameters() {
    ...
    currentPlayer = gameState[numberOfPlayers] = alphabet.indexOf(input.charAt(input.length - 9))
...
function startup(players, online) {
    ...
    currentPlayer = gameState[numberOfPlayers]
    id("playerView").innerHTML = currentPlayer + 1
    allowEndTurn = true
...
let spectatedPlayer = currentPlayer + 1

function nextSpectatedPlayer() {
    spectatedPlayer++
    if (spectatedPlayer == numberOfPlayers) {
        spectatedPlayer = 0
    }
    if (spectatedPlayer == currentPlayer) {
        spectatedPlayer++
    }
    if (spectatedPlayer == numberOfPlayers) {
        spectatedPlayer = 0
...
function endTurn() {
    ...
    if (!isOnline) {
        ...
        if (currentPlayer == numberOfPlayers - 1) {
            currentPlayer = 0
            spectatedPlayer = currentPlayer + 1
            if (gameState[currentPlayer][5].length == 5) {
                allowFactory = false
            } else {
                allowFactory = true
            }
        ...
        setResilience = false
        setSpecial = false
        for (let i = 0; i < gameState[currentPlayer][1][6].length; i++) {
            gameState[currentPlayer][1][6][i] = 0
        }
        updateBoards()
    } else {
        copyLink("")
...
function enterCard(player, cardNumber) {
    if (enteredCards !== gameState[currentPlayer][5].length) {
...
function createAsset(strength, resilience, special) {
    ...
    gameState[currentPlayer][1][0].push(Strength)
    assetValues.splice(0, 1)
    gameState[currentPlayer][2].splice(strength, 1)

    gameState[currentPlayer][1][1].push(Resilience)
    gameState[currentPlayer][1][4].push(Resilience)
    assetValues.splice(0, 1)
    if (strength < resilience) {
        gameState[currentPlayer][2].splice(resilience - 1, 1)
    } else {
        gameState[currentPlayer][2].splice(resilience, 1)
    }

    if (special !== null) {
        gameState[currentPlayer][1][2].push(Special)
        assetValues.splice(0, 1)
        if (strength < special && resilience > special) {
            gameState[currentPlayer][2].splice(special - 1, 1)
        } else if (strength < special && resilience < special) {
            gameState[currentPlayer][2].splice(special - 2, 1)
        } else if (strength > special && resilience > special) {
            gameState[currentPlayer][2].splice(special, 1)
        }
    } else {
        gameState[currentPlayer][1][2].push(0)
    }

    gameState[currentPlayer][1][3].push(0)
    gameState[currentPlayer][1][6].push(0)
    updateBoards()
}
...
function updateBoards() {
    ...
    id("playerBoard" + currentPlayer).style.display = "block"

    //enemy health
    for (let i = 0; i < numberOfPlayers; i++) {
        createElement("DIV", "playerBoard" + i, "Player " + (spectatedPlayer + 1) + "'s health: " + gameState[spectatedPlayer][4], "enemyHealth" + i)
    }
    // enemy factories
    for (let i = 0; i < numberOfPlayers; i++) {
        createElement("DIV", "playerBoard" + i, null, "enemyFactories" + i, "factories-container")
        if (gameState[spectatedPlayer][5].length > 0) {
            createElement("P", "enemyFactories" + i, "Player " + (spectatedPlayer + 1) + "'s factories: ")
        } else {
            createElement("P", "enemyFactories" + i, "Player " + (spectatedPlayer + 1) + " has no factories")
        }
        for (let e = 0; e < gameState[spectatedPlayer][5].length; e++) {
            var card = gameState[spectatedPlayer][5][e]
            if (card == 1) {
                card = "Ace"
            }
    ...
    for (let i = 0; i < numberOfPlayers; i++) {
        ...
        if (gameState[spectatedPlayer][1][0].length == 0) {
            createElement("P", "enemyAssets" + i, "Player " + (spectatedPlayer + 1) + " has no assets", "enemyAssetText")
        } else {
            createElement("P", "enemyAssets" + i, "Player " + (spectatedPlayer + 1) + "'s assets:", "enemyAssetText")
        }
        for (let e = 0; e < gameState[spectatedPlayer][1][0].length; e++) {

            var special = gameState[spectatedPlayer][1][2][e]
            if (special == 11) {
                special = "Jack"
            } else if (special == 13) {
                special = "King"
            ...
            createElement("P", i + "enemy-asset-container" + e, gameState[spectatedPlayer][1][0][e], i + "enemy-asset-strength" + e, "asset-text")
            createElement("P", i + "enemy-asset-container" + e, special, i + "enemy-asset-special" + e, "asset-text")
            createElement("P", i + "enemy-asset-container" + e, gameState[spectatedPlayer][1][4][e], i + "enemy-asset-resilience" + e, "asset-text")

            if (gameState[spectatedPlayer][1][6][e] == 1) {
                id(i + "enemy-asset-container" + e).classList.add("tapped")
            }
            if (gameState[spectatedPlayer][1][3][e] == 1) {
                id(i + "enemy-asset-container" + e).classList.add("attacking")
            }
        }
    }
    ...
    for (let i = 0; i < numberOfPlayers; i++) {
        ...
        for (let e = 0; e < gameState[i][1][0].length; e++) {
            ...
            createElement("DIV", "assetCards" + i, null, i + "asset-container" + e, "asset", null, function r() {
                if (gameState[i][1][6][e] == 0) {
                    if (gameState[i][1][3][e] == 0) {
                        gameState[i][1][3][e] = 1
                        id(i + "asset-container" + e).classList.add("attacking")
                        console.log(gameState[i][1][0][e], gameState[i][1][2][e], gameState[i][1][4][e])
                    } else if (gameState[i][1][3][e] == 1) {
                        updateBoards()
                        for (let i = 0; i < gameState[currentPlayer][1][3].length; i++) {
                            gameState[currentPlayer][1][3][i] = 0
                        }
                    }
                }
            })
            ...
            if (gameState[currentPlayer][1][6][e] == 1) {
    ...
    for (let i = 0; i < numberOfPlayers; i++) {
        ...
        for (let e = 0; e < gameState[i][2].length; e++) {
            ...
            id(i + "enteredCards" + e).addEventListener("click", function () {
                if (assetValues.length < 3) {
                    storeAsset(0, e, gameState[currentPlayer][2][e])
        ...
        createElement("SPAN", "entered" + i, "+", "addEntered", "card", null, function e() {
            ...
            setResilience = false
            setSpecial = false
            enterCard(currentPlayer, handSelected)
            updateBoards()
        })
    }
    //factories
    for (let i = 0; i < numberOfPlayers; i++) {
        ...
        createElement("SPAN", "factories" + i, "+", "addFactory", "card", null, function e() {
            ...
            setResilience = false
            setSpecial = false
            createFactory(currentPlayer, handSelected)
            updateBoards()
        })
    }
    //hands
    ...
    for (let i = 0; i < numberOfPlayers; i++) {
        ...
        id("deckCards" + i).addEventListener("click", function () {
            drawCard(currentPlayer, setDrawLimit())
        })
    }
    //player healths
    for (let i = 0; i < numberOfPlayers; i++) {
        createElement("DIV", "playerBoard" + i, "Player " + (currentPlayer + 1) + ", your health: " + gameState[i][4], "health" + i)
    }
}

//setting up attacking cards
function setupAttacking() {
    for (let i = 0; i < gameState[currentPlayer][1][3].length; i++) {
        if (gameState[currentPlayer][1][3][i] == 1) {
            console.log(gameState[currentPlayer][1][0][i], gameState[currentPlayer][1][2][i], gameState[currentPlayer][1][1][i])
            gameState[currentPlayer][1][5][0][i] = spectatedPlayer
            gameState[currentPlayer][1][6][i] = 1
        }
    }

    if (assetsAreAttacking()) {
        showMessage("you are attacking player " + (spectatedPlayer + 1) + " with " + gameState[currentPlayer][1][3].length + " card(s)")
        gameState[numberOfPlayers + 2] = spectatedPlayer


    } else {
        showMessage("Select cards to attack")
    }
}
//checks if any number of assets are attacking
function assetsAreAttacking() {
    for (let i = 0; i < gameState[currentPlayer][1][3].length; i++) {
        if (gameState[currentPlayer][1][3][i] == 1) {
            return true
            break
...
function storeAsset(yes, e, cardValue) {
    if (gameState[currentPlayer][2].length >= 2) {
        ...
        if (yes == 0) {
            if (setStrength) {
                if (cardValue < 11) {
                    ...
                    strengthInput = e
                    document.getElementById(currentPlayer + "enteredCards" + e).classList.add("highlight")
                    setStrength = false
                    setResilience = true
...
function showMessage(message) {
    createElement("P", "textBox", "Game Says to Player " + (currentPlayer + 1) + ": " + message)
