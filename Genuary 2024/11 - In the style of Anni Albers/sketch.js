let fabricTexture
function preload() {
    fabricTexture = loadImage("canvasEffect.png")
}
...
function setup() {
    ...
    createCanvas(c.width * 20, c.height * 20)
    pixelDensity(1)
    noSmooth()
...
function draw() {
    ...
    const colorHSL = [random(), 1, 0.3]
    const color = hslToRgb(colorHSL[0], colorHSL[1], colorHSL[2])
    const colorLight = hslToRgb(colorHSL[0], colorHSL[1], colorHSL[2] * 2.5)
    const colorWhite = hslToRgb(colorHSL[0], colorHSL[1], 0.96)
    ...
    blendMode(MULTIPLY) //add an image on top to make it look like a fabric
